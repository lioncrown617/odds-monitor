...
def calc_top3():
    data    = state["data"]; base = state["base_data"]
    sms_map = state["_sms"]; absorbs = state["_absorb"]
    tc      = state["trend_counter"]
    cd      = state["cum_drop"]; cr = state["cum_rise"]
    cum_f   = state["cum_flow"]
    down, up, suspicious, steady_list, rci_list = [], [], [], [], []

    for r in data:
        no     = r["no"]; sms = sms_map.get(no, 0.0)
        ab     = absorbs.get(no, {}); cum_in = cum_f.get(no, 0.0)

        streak_d = max(tc.get(no, 0), 0); drop = cd.get(no, 0)
        if streak_d > 0 or drop > 0:
            down.append({"no": no, "name": r["name"], "win": r["win"],
                         "base": base.get(no, "—"), "streak": streak_d, "drop": drop,
                         "cum_inflow": round(cum_in), "sms": sms,
                         "excess": ab.get("excess", 0)})

        streak_u = max(-tc.get(no, 0), 0); rise = cr.get(no, 0)
        if streak_u > 0 or rise > 0:
            up.append({"no": no, "name": r["name"], "win": r["win"],
                       "base": base.get(no, "—"), "streak": streak_u, "rise": rise})
        ...
        try:
            ...
            if steady > 0:
                steady_list.append({"no": no, "name": r["name"], "win": r["win"],
                                    "base": base.get(no, "—"),
                                    "cum_inflow": round(cum_in), "steady": steady})
        except: pass

        try:
            rise_pct2 = cr.get(no, 0.0); rci_score = calc_rci(no, cum_in, rise_pct2)
            if rci_score > 0:
...
def append_snapshot(now, data, est_bets, flows, absorbs, sms, win_pool):
    ...
    if not log["meta"]:
        log["meta"] = {
            "race_date":  state["race_date"], "venue": state["venue"],
    ...
    for r in data:
        ...
        snapshot["horses"].append({
            ...
            "sms":        sms.get(no, 0), "rci": rci_s, "steady": st,
    ...
    for a in state["alerts"]:
        ...
        if k not in existing:
            log["alerts"].append(a); existing.add(k)
    _save_log(log)
...
def get_data():
    ...
    for r in state["data"]:
        ...
        if flow > 0:
            ...
            flow_str = f"▲ +{fmt_money(flow)}{rescue_tag}"
            flow_css = "up"
        elif flow < 0:
