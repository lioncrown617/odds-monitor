...
state = {
    ...
    'timestamps': [], 'win_pool': '', 'win_pool_history': [], '_accels': {}, '_absorb': {}, '_sms': {}, '_alerts': {},
    'e_history': defaultdict(_deque10), 'inflow_ts_history': defaultdict(_deque60), 'min_odds': defaultdict(_inf),
    'alert_cooldown': defaultdict(dict), 'steady_scores': {}, 'last_error_detail': '', 'race_info': {},
    ...
    for r in data:
        ...
        try:
            ...
            if no in base:
                ...
                if pct > 0:
                    cd[no] = round(pct, 1); cr[no] = 0.0
                else:
                    cr[no] = round(abs(pct), 1); cd[no] = 0.0
        except:
            pass
...
def calc_sms_v2(no, cum_flow_val, cum_drop_val):
    ...
    recent_pos_ratio = sum(1 for x in last6 if x > 0) / len(last6) if last6 else 0.0
...
def calc_acc_meta(no):
    ...
    big_entries = [(ts, amt) for ts, amt in inflow_hist if amt >= 10000]
    if len(big_entries) < 2:
        return {'batch_count': 0, 'time_span': 0.0, 'max_gap_min': 0.0}
...
def calc_top3():
    ...
    sms_all = []
    for r in data:
        no = r['no']; sms_score = sms_map.get(no, 0.0); ab = absorbs.get(no, {})
        cum_in = cum_f.get(no, 0.0); trend_v = tc.get(no, 0)
        ...
        if sms_score > 0 and trend_v >= -1 and not weakening:
            sms_all.append({
                'no': no, 'name': r['name'], 'win': r['win'], 'base': base.get(no, '—'),
    ...
    if new_top:
        state['top_down'] = new_top
    state['top_up'] = []
    acc_list = []
    for r in data:
        no = r['no']; cum_in = cum_f.get(no, 0.0); acc_s = calc_acc_score(no, cum_in)
...
def get_log_path():
    ...
    venue = state['venue']
    race_no = str(state['race_no']).zfill(2)
...
def append_snapshot(now, data, est_bets, flows, absorbs, sms, win_pool):
    ...
    if not log['meta']:
        log['meta'] = {
            'race_date': state['race_date'], 'venue': state['venue'], 'venue_name': state['venue_name'],
            'race_no': state['race_no'], 'base_time': state['base_time'], 'start_time': now,
    ...
    for r in data:
        ...
        snapshot['horses'].append({
            'no': r['no'], 'name': r['name'], 'win': r['win'], 'place': r.get('place', ''),
            ...
            'absorb_pct': ab.get('absorb_pct', 0), 'excess': ab.get('excess', 0),
            'pool_inc': ab.get('pool_inc', 0), 'odds_drop': ab.get('odds_drop', 0),
            'cum_drop': state['cum_drop'].get(no, 0), 'cum_rise': state['cum_rise'].get(no, 0),
            'sms': sms.get(no, 0), 'acc': sms.get(f'acc_{no}', 0),
...
def finalize_log(now):
    ...
    log['summary'] = {
        'end_time': now, 'total_updates': state['update_count'], 'base_time': state['base_time'],
...
def build_data_payload():
    ...
    winbig = []
    for r in state['data']:
        no = r['no']; label, css = get_trend_label(no)
        ...
        try:
            ...
            if real_amt > 0:
                estamt = real_amt; estbet = fmt_money(estamt)
            else:
                share = (1.0 / float(r['win'])) / total_inv if total_inv > 0 else 0
        ...
        try:
            if base_amt > 0:
                ...
                pv = cdval / base_amt * 100; sign2 = '+' if pv > 0 else ''
                cumdiff_pct = f'{sign2}{pv:.1f}%'
        except:
            pass
        cum_in = state['cum_flow'].get(no, 0.0)
        cum_in_str = fmt_money(cum_in) if cum_in > 0 else ''
        ab = state['_absorb'].get(no, {})
        flow = ab.get('flow', 0); absorb_pct = ab.get('absorb_pct', 0.0)
        excess = ab.get('excess', 0.0); pool_inc = ab.get('pool_inc', 0)
        odds_drop = ab.get('odds_drop', 0.0); odds_accel = ab.get('odds_accel', 0.0)
        is_rescue = ab.get('is_rescue', False)
        if flow > 0:
            rescue_tag = '🆘' if is_rescue else ''
            flow_str, flow_css = f'+{fmt_money(flow)}{rescue_tag}', 'up'
        elif flow < 0:
            flow_str, flow_css = f'-{fmt_money(abs(flow))}', 'diluted'
        else:
        ...
        if odds_accel > 1 and odds_drop > 1:
            odrop_str, odrop_css = f'-{odds_drop:.1f}%', 'hot'
        elif odds_drop > 0:
        ...
        rows.append({
            **{k: r.get(k, '') for k in ['no', 'name', 'barrier', 'jockey', 'trainer', 'win', 'place']},
            'basewin': base_win, 'prevwin': prev_win, 'chg': chg_str,
            'trend': label, 'trendcss': css, 'estbet': estbet, 'estpct': estpct,
            'basebet': base_bet_str, 'cumdiff': cumdiff_str, 'cumdiffcss': cumdiff_css,
            'cumdiffpct': cumdiff_pct, 'cuminflow': cum_in_str, 'flow': flow_str,
            'flowcss': flow_css, 'flowraw': round(flow), 'accel': accel_str,
            'accelcss': accel_css, 'absorb': absorb_str, 'absorbcss': absorb_css,
            'odrop': odrop_str, 'odropcss': odrop_css, 'sms': sms_str, 'smscss': sms_css,
            'smsraw': sms_score, 'accraw': acc_score, 'alert': alert_str,
            'issuspicious': False, 'risefrommin': round(rise_from_min, 1),
        })
        score = state['cum_flow'].get(no, 0.0) + max(flow, 0)
        if score > 0:
            winbig.append({'no': no, 'name': r['name'], 'flow': fmt_money(score), 'raw': score})
    winbig = sorted(winbig, key=lambda x: x['raw'], reverse=True)[:8]
    return {
        'rows': rows, 'winbig': winbig, 'topdown': state['top_down'], 'topup': state['top_up'], 'topacc': state['top_acc'],
        'alerts': [], 'updatecount': state['update_count'], 'lastupdate': state['last_update'], 'basetime': state['base_time'],
        'status': state['status'], 'haserror': state['has_error'], 'running': state['running'], 'racedate': state['race_date'],
        'venuename': state['venue_name'], 'raceno': state['race_no'], 'interval': state['interval'], 'currentinterval': state['current_interval'],
        'history': {k: v for k, v in state['history'].items()}, 'bethistory': {k: v for k, v in state['bet_history'].items()},
        'flowhistory': {k: v for k, v in state['flow_history'].items()}, 'absorbhistory': {k: v for k, v in state['absorb_history'].items()},
        'smshistory': {k: v for k, v in state['sms_history'].items()}, 'acchistory': {k: v for k, v in state['acc_history'].items()},
        'timestamps': state['timestamps'], 'horses': {r['no']: r['name'] for r in state['data']},
        'winpool': state['win_pool'], 'winpoolhistory': state['win_pool_history'], 'errordetail': state.get('last_error_detail', ''),
...
def get_qin_flow():
    ...
    try:
        ...
        resp = requests.get(qin_api, params={'date': state['race_date'], 'venue': state['venue'], 'raceno': state['race_no']}, timeout=4)
        resp.raise_for_status()
        data = resp.json()
        if not data.get('ok'):
        ...
        def process(pool_data, prev_map, history_key):
            ...
            for item in pool_data.get('odds', []):
                combo = item['combo']
                try:
                    curr_inv = float(item.get('investment', 0) or 0)
                except:
                    curr_inv = 0.0
                ...
                if prev_inv is not None:
                    ...
                    if flow >= 1000:
                        entries.append({'time': now_str, 'combo': combo.replace('+', '-'), 'odds': item.get('odds', ''), 'flow': flow, 'raw': flow})
        ...
        return jsonify({'ok': True, 'qin': tier(state['qin_history']), 'qpl': tier(state['qpl_history'])})
    except Exception as e:
