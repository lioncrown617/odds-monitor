...
state = {
    ...
    "sms_history": defaultdict(list), "acc_history": defaultdict(list),
...
def fmt_money(amt):
    ...
    elif a >= 1_000:   return f"${a/1_000:.1f}K"
    return f"${a:.0f}"
...
def calc_sms_v2(no, cum_flow_val, cum_drop_val):
    ...
    if len(recent_flows)>=3 and all(f<=0 for f in recent_flows): Wt*=0.55
    if len(recent_flows)>=3:
        al = [recent_flows[i]-recent_flows[i-1] for i in range(1,len(recent_flows))]
        if all(f<0 for f in recent_flows) and all(a<0 for a in al): Wt*=0.75
...
def calc_acc_meta(no):
    inflow_hist = list(state["inflow_ts_history"][no])
    big_entries = [(ts,amt) for ts,amt in inflow_hist if amt>=10000]
    if len(big_entries) < 2: return {"batch_count":0,"time_span":0.0,"max_gap_min":0.0}
...
def calc_top3():
    ...
    absorbs=state["_absorb"]; tc=state["trend_counter"]
    cd=state["cum_drop"]; cum_f=state["cum_flow"]
    sms_all=[]
    for r in data:
...
def get_log_path():
    date_str=state["race_date"].replace("-",""); venue=state["venue"]
    race_no=state["race_no"].zfill(2); os.makedirs("logs",exist_ok=True)
...
def finalize_log(now):
    ...
    horses=[{
        "no":no,"name":next((r["name"] for r in state["data"] if r["no"]==no),no),
    ...
    log["summary"]={"end_time":now,"total_updates":state["update_count"],
        "base_time":state["base_time"],"final_pool":state["win_pool"],"total_alerts":0,
...
def get_data():
    ...
    for r in state["data"]:
        ...
        base_win=state["base_data"].get(no,"—"); prev_win=state["prev_data"].get(no,"")
        chg_str=""
        try:
    ...
    return jsonify({
        ...
        "basetime":state["base_time"],"status":state["status"],
        "haserror":state["has_error"],"running":state["running"],
        "racedate":state["race_date"],"venuename":state["venue_name"],
        "raceno":state["race_no"],"interval":state["interval"],
        "currentinterval":state["current_interval"],
